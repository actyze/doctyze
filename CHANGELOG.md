...
- **`write-product-docs` skill + `docs/product/` section.** A **curated, narrative product-
  ...
  - **Curated, not sprawling:** ONE guide per product area (covering several capabilities),
    a handful of docs per repo — explicitly **not** one-per-capability.
  - **Substantial, not thin:** each feature guide follows a depth bar (what · when · how ·
    **grounded worked examples** · options & limits · edge cases · technical-detail link), plus a
    concepts+glossary page and a use-cases page. Worked examples MUST trace to real values in the
    code / specs / test fixtures — never invented.
